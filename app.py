...
# @st.cache_resource
# def load_model_and_vectorizer():
#     with open('savedPackages/model.pkl', 'rb') as model_file:
#         model = pickle.load(model_file)
#     with open('savedPackages/vectorizer.pkl', 'rb') as vectorizer_file:
#         vectorizer = pickle.load(vectorizer_file)
#     return model, vectorizer
#
#
...
with open("tokenizer_for_lstm_glove_true.pkl", "rb") as f:
    tokenizer = pickle.load(f)

# Parameters
MAX_LEN = 100  # Use the same max length used during training
...
def predict_sentiment(text):
    ...
    pred = model.predict(padded)[0]  # e.g., [0.02, 0.10, 0.88]
    class_labels = ["Negative", "Neutral", "Positive"]
    result = {label: float(f"{confidence:.4f}") for label, confidence in zip(class_labels, pred)}
    return result
